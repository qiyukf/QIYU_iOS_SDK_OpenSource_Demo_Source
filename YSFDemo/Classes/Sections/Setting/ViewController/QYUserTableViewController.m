...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    [[QYSDK sharedSDK] trackHistory:@"个人信息" enterOrOut:YES key:_key];
}

- (void)viewDidDisappear:(BOOL)animated
{
    [super viewDidDisappear:animated];
    [[QYSDK sharedSDK] trackHistory:@"个人信息" enterOrOut:NO key:_key];
    self.key = [[NSUUID UUID] UUIDString];
}
...
- (void)testTap:(id)sender
{
    if (!isTestMode) {
        isTestMode = YES;
        [self.view ysf_makeToast:@"Test Mode" duration:1.0 position:YSFToastPositionCenter];
    } else {
        isTestMode = NO;
        [self.view ysf_makeToast:@"Normal Mode" duration:1.0 position:YSFToastPositionCenter];
    }
}
...
{
    NSString *name = [QYUserTableViewController selectRowByIndex:sender.index];
...
+ (NSString *)selectRowByIndex:(NSString *)index
{
